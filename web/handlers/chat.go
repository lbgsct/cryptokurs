...
import (
	...
	"github.com/lbgsct/cryptokurs/algorithm" // Импорт пакета algorithm
...
func ChatHandler(c *gin.Context) {
	...
	for _, msg := range historyResp.Messages {
		...
		newMsg := *msg
		newMsg.EncryptedMessage = decryptedMessage
...
func SendMessageHandler(c *gin.Context) {
	...
	if strings.TrimSpace(message) == "" {
		c.HTML(http.StatusBadRequest, "error.html", gin.H{"error": "Нельзя отправить пустое сообщение"})
		return
	}
	...
	if cipherContext == nil {
		c.HTML(http.StatusBadRequest, "error.html", gin.H{"error": "Шифровальный контекст ещё не инициализирован"})
		return
	}
	...
	if err != nil {
		...
		c.HTML(http.StatusInternalServerError, "error.html", gin.H{"error": "Ошибка шифрования сообщения"})
		return
	}

	// Отправляем зашифрованное сообщение через gRPC
	...
	if err != nil {
		...
		c.HTML(http.StatusInternalServerError, "error.html", gin.H{"error": "Ошибка отправки сообщения"})
		return
	}
...
func receiveMessages(
	...
	for {
		...
		if msg.GetType() == "file" {
			// Обработка файловых сообщений аналогично
			// Расшифровка фрагментов файла с использованием cipherContext
		}
	}
}
